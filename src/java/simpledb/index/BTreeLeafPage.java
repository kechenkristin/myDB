...
public class BTreeLeafPage extends BTreePage {
    ...
    public Iterator<Tuple> iterator() {
        return new BTreeLeafPageForwardIterator(this);
    }
...
/**
 * Abstract class that implements Java Iterator for tuples on a BTreeLeafPage.
 */
abstract class BTreeLeafPageIterator implements Iterator<Tuple> {
    int curTuple = 0;
    ...
    protected boolean hasNextHelper(int destination, int step) {
        if (nextToReturn != null)
            return true;
        try {
            return findNextTuple(curTuple, destination, step);
        } catch (NoSuchElementException e) {
            return false;
        }
    }
    ...
    protected boolean findNextTuple(int startTuple, int endTuple, int step) {
        for (int i = startTuple; i != endTuple; i += step) {
            Tuple tuple = p.getTuple(i);
            if (tuple != null) {
                nextToReturn = tuple;
                curTuple = i + step;
                return true;
...
/**
 * Helper class that implements the Java Forward Iterator for tuples on a BTreeLeafPage.
 */
class BTreeLeafPageForwardIterator extends BTreeLeafPageIterator {
    public BTreeLeafPageForwardIterator(BTreeLeafPage p) {
        super(p);
    }
    ...
    public boolean hasNext() {
        return hasNextHelper(p.getMaxTuples(), 1);
    }
}
 ...
 */
class BTreeLeafPageReverseIterator extends BTreeLeafPageIterator {
